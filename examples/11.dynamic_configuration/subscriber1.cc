...
void process_update(ConfigItem *config) {
	...
	{
		if (validate(config->data) < 0) 
		{
			Debug::log(
				"thread {} Validation failed for {}", thread_id_get(), config->data);
		}
		else 
		{
	...
	print_config(CONFIG1, configData);
...
void __cheri_compartment("subscriber1") init()
{
	...
	auto config = get_config(READ_CONFIG_CAPABILITY(CONFIG1));
	if (config == nullptr) {
		Debug::log("thread {} failed to get {}", thread_id_get(), CONFIG1);
		return;
	}
	...
	while (true)
	{
		Timeout t1{MS_TO_TICKS(5000)};
		if (futex_timed_wait(&t1, &(config->version), configVersion) == 0)
		{
			Debug::log("thread {} get value for {}", thread_id_get(), CONFIG1);
			auto config = get_config(READ_CONFIG_CAPABILITY(CONFIG1));
			Debug::log("thread {} got version:{} {}", thread_id_get(), config->version, config->data);
